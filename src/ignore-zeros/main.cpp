...
int main(int argc, char **argv) {
  ...
  for (int m = m_min; m <= m_max; m++) {
    ...
    real* log_normalizer = new real[a_max];
    ...
    tic = clock();
    // we keep track of the first and last non-zero values in the current iteration
    ...
    for (int s = 3; s < a_max; s++) {
      ...
      // let us update first and last non-zero
      for (; first_non_zero < a_max; first_non_zero++) {
        if (current[0][first_non_zero] != 0.0) {
          printf("non-zero = %7d, ", first_non_zero);
          break;
        }
      }
      int last_non_zero = a_max;
      for (; first_non_zero < a_max; first_non_zero++) {
        if (current[0][first_non_zero] != 0.0) {
          printf("non-zero = %7d, ", first_non_zero);
          break;
