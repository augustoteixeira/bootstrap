...
int main(int argc, char **argv) {
  ...
  for (int m = m_min; m <= m_max; m++) {
    ...
    for (int s = 3; s < a_max; s++) {
      ...
      fprintf(bump, "%d, %14.7f\n", s, log_normalizer[s + 1]);
      ...
      if (s == a_max - 1) {
        ...
        printf("log(p) = %7.6f, -p log(sum) = %7.6f # m = %d, p = %7.6f, a = %d, nz = %d, time = %f\n",
               -log(p), -p * (log(sum) + log_normalizer[s]), m, p, a_max, min_nonzero, (float)(clock() - tic)/CLOCKS_PER_SEC);
        fflush(stdout);
        fprintf(fpt, "%d, %14.7f\n", m, -p * (log(sum) + log_normalizer[s]));
      }
    }
    free(N0);
    free(N1);
...
//     // trim small values
//     if (output6[a] < TRUNCATE_LOW) { output6[a] = 0.0; }
//   }
// }
  ...
  real* ln_next)
{
  ...
  real convert_from_3 = exp(ln3 - ln0);
  real convert_from_4 = exp(ln4 - ln0);
  ...
  for (int a = min_nonzero; a < s; a++) {
    ...
    // trim small values
    if (output[0][a] < TRUNCATE_LOW) { output[0][a] = 0.0; }
    // update running max
  ...
  *ln_next = log(max) + ln0;
  for (int a = min_nonzero; a < s; a++) {
    ...
    // trim small values
    if (output[1][a] < TRUNCATE_LOW) { output[1][a] = 0.0; }
  }
  ...
  for (int a = min_nonzero; a < s; a++) {
    ...
    // trim small values
    if (output[6][a] < TRUNCATE_LOW) { output[6][a] = 0.0; }
  }
  for (int a = min_nonzero; a < s; a++) {
    int b = s - a;
    output[5][a]
    ...
    // trim small values
    if (output[5][a] < TRUNCATE_LOW) { output[5][a] = 0.0; }
  }
  for (int a = min_nonzero; a < s; a++) {
    int b = s - a;
    output[4][a]
      ...
      + np[a] * output[5][a];                             // 5 -> 4
    // trim small values
    if (output[4][a] < TRUNCATE_LOW) { output[4][a] = 0.0; }
  }
  for (int a = min_nonzero; a < s; a++) {
    int b = s - a;
    output[2][a]
      ...
      + np[b] * output[6][a];                             // 6 -> 2
    // trim small values
    if (output[2][a] < TRUNCATE_LOW) { output[2][a] = 0.0; }
  }
  for (int a = min_nonzero; a < s; a++) {
    int b = s - a;
    output[3][a]
      ...
      + np[b] * output[4][a];                             // 4 -> 3
    // trim small values
    if (output[3][a] < TRUNCATE_LOW) { output[3][a] = 0.0; }
  }
