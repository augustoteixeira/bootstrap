...
lp = np.log(pp[6:-1]) # trim first points
ll = np.log(ls[6:-1])
...
pinv = np.divide(1, pp[8:-1])
plst = pls[8:-1]
def polydecay(x, a, b, beta_plus_one):
    return a - b / pow(x, beta_plus_one)

popt, pcov = curve_fit(polydecay, pinv, plst, p0=[3.2, 10, 0.3])
...
a = math.pi**2/3
a_minus_log_pls = np.log(a - pls[6:-1])
...
am_over_sqrt = np.divide(a_minus_log_pls, np.sqrt(pp[6:-1]))
pinv_2 = np.divide(1, pp[6:-1])
