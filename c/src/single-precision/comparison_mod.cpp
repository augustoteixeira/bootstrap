...
void c_modifiedf(float p, int s, void*, void*, void*, int, real, real, real);
...
int main(int argc, char **argv) {
  ...
  for (int m = m_min; m <= m_max; m++) {
    ...
    real* log_normalizerf = new real[a_max + 1];
    ...
    for (int s = 3; s < a_max; s++) {
      ...
      for (int l = 0; l < a_max; l++) {
        ...
        if (maxf < (float) currentf[0][l]) { maxf = (float) currentf[0][l]; }
      }
      log_normalizer[s + 1] = log(max) + log_normalizer[s];
      log_normalizerf[s + 1] = log(maxf) + log_normalizerf[s];

      fprintf(fpt,"%f, %f, %f, %f\n", max, maxf, log_normalizer[s + 1], log_normalizerf[s + 1]);

      if (s == 3129) {
        for (int l = 0; l < s; l++) {
          fprintf(fpt2,"%f, %f\n", current[0][l], currentf[0][l]);
        }
      }
