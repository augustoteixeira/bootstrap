...
real normalize(real v) { return fmax(log(v), -200); }
...
int main(int argc, char **argv) {
  ...
  real p_for_image = pow(2.0, -m_for_image);
  int a_max_for_image = (int) LOG_MULTIPLE * log(1.0 / p_for_image) / p_for_image;
    ...
    for (int s = 3; s < a_max; s++) {
      ...
      if (s == a_max - 1) {
        ...
        printf("%f, %f # m = %d, p = %7.6f, a = %d, time = %f\n",
               -log(p), -p * log(sum), m, p, a_max, (real)(clock() - tic)/CLOCKS_PER_SEC);
        //printf("p = %7.6Lf, size = %7d, -p * log(diagonal sum) = %7.6Lf, m = %2d\n",
        //       p, a_max,-p * log(sum), m);
        //printf("m = %d, N = %d, log(sum{M(x, N-x)}) = %f, -log(middle) = %f\n",
        //       m, a_max, log(sum), -log(current[s/2]));
        fflush(stdout);
        if (m == m_for_image){
          // print tables
          if (display_table) {
